...
if [[ $1 == "all" ]]; then
	rm -rf linux musl-$MUSL_VERSION* dash-$DASH_VERSION* sbase ubase sinit rc.shutdown/rc.shutdown sysroot initrd slinux.iso
elif [[ $1 == "userland" ]]; then
	rm -rf musl-$MUSL_VERSIONl* dash-$DASH_VERSION* sbase ubase sinit rc.shutdown/rc.shutdown sysroot initrd slinux.iso
elif [[ $1 == "initrd" ]]; then
